package com.paytm.shinra.export
...
/* Spark Submit:
  ...
  /bin/spark-submit --class com.paytm.shinra.export.ExportParquetToNativeES \
    --name "ExportParquetToNativeES" \
    --master "yarn-cluster" \
    --queue "Midgar" \
    --driver-memory 4g \
    --num-executors 10 \
 ...
 */
object ExportParquetToNativeES extends ExportParquetToNativeESJob with SparkJob with SparkJobBootstrap {
}

trait ExportParquetToNativeESJob {
  this: SparkJob =>

  val JobName = "ExportParquetToNativeES"
  ...
  def runJob(spark: SparkSession, settings: Settings, args: Array[String]): Unit = {
    ...
    val esPathToParquet = {
      if (args.length > ARG_PATH_TO_PARQUET)
        ...
        featuresDfs.exportTable + "snapshot_delta/" + targetDateStr
    }

    val esIndex = replaceDateTemplate(index, targetDate)

    log.info {
      s"""ExportParquetToNativeES
          |
      |Configurations:
