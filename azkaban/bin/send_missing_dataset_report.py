...
class global_vars:
    ...
    file_type  = "csv"
    local_path_format = "/tmp/MissingDatasetsReport_dt_%s.%s"
...
def getHTMLHeader():
    ...
    html_preface = "<h3>Missing Datasets Report</h3><br/>"
...
def getHTMLTable(rows, csvTestDate):
    ...
    tableItems = [("Sick" in row, ["<td>{}</td>".format(item) for item in row.split(",")]) for row in data]
    tableRows = "\n".join([getRow(failedFlag, row) for failedFlag, row in tableItems])
    tableHeader =  "\n".join(["<th>{}</th>".format(item) for item in header.split(",")])

    emailHeading ="""
    <h2>{}</h2>
    <p>Please investigate the health status of following raw datasets</p>
    <table id="customers">
    <tr>""".format("Raw Dataset Health Report Date: %s"%(csvTestDate))
...
def sendMail(to_emails, cc_emails, subject, csvFileName, csvDate):
    ...
    fileName = "/tmp/missing_dataset_report_mail_{}.html".format(str(uuid.uuid4()))
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Sends missing dataset report Emails')
